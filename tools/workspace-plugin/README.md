...
- `ai=true` materializes only the existing provider-neutral Phase 14 model, typed-tool, evaluation, durable-execution, and governance capabilities into the API dependency graph and generates `apps/api/src/app/ai/reference-workflow.ts`, its focused tests, and its production-replacement guidance. It does not install a model-provider SDK or orchestration framework.
