...
function configureApiDependencies(tree: Tree, options: AiProfileOptions): void {
  ...
  for (const capability of aiCapabilities) {
    const dependency = packageName(options.packageScope, capability.packageSuffix);
...
function referenceWorkflowSource(packageScope: string): string {
  return `import {
  ...
  return {
    ...
    inputSchema: {
      parse(value: unknown): ReferenceLookupInput {
        ...
        if (!exactKeys(input, ['key']) || typeof input.key !== 'string' || !input.key.trim()) {
    ...
    outputSchema: {
      parse(value: unknown): ReferenceLookupOutput {
        ...
        const allowed = output.value === undefined ? ['found'] : ['found', 'value'];
        if (
...
function modelClient(config: ReferenceAiWorkflowConfig, route: ModelRoute): ModelClient {
  const client = config.modelClients[route.routeId];
  if (!client) throw new Error('No server-owned model client is configured for route ' + route.routeId + '.');
  return client;
}
...
async function failRun(session: DurableRunSession, code: string): Promise<void> {
...
export async function* runReferenceAiWorkflow(
  ...
  const routeRequirements = {
    ...
    ...(config.allowedRegions === undefined ? {} : { allowedRegions: config.allowedRegions }),
  ...
  const coordinator = new DurableExecutionCoordinator(config.persistence, {
    now,
    ...(config.durableObserver === undefined ? {} : { observer: config.durableObserver }),
  });
  ...
  let completion: { readonly finishReason: 'stop' | 'length' | 'content_filter' | 'unknown'; readonly usage: ModelUsage } | null = null;
  ...
  try {
    for (;;) {
      ...
      await session.checkpoint({
        checkpointId: request.runId + '-input-' + String(checkpointSequence + 1),
      ...
      try {
        for await (const event of modelClient(config, route).stream({
          ...
          if (event.type === 'text_delta') {
            const safeOutput = await runWithCorrelationContext(correlation, () =>
              applyOutputPolicy(
                {
                  traceId: request.traceId,
                  actorId: request.actorId,
                ...
                { value: event.text, classification: safeInput.classification },
                config.outputPolicy,
                config.audit,
              ),
            );
    ...
    if (completion === null) throw new Error('Model stream completed without a terminal event.');
    ...
    if (
      ...
      await session.pauseForApproval({
        checkpoint: {
          checkpointId: request.runId + '-approval-' + String(checkpointSequence + 1),
          sequence: ++checkpointSequence,
    ...
    runWithCorrelationContext(correlation, () =>
      logger.error('ai.reference.failed', error, { runId: request.runId, code }),
    );
    yield {
      ...streamBase(sequence++, request, route, now),
      type: 'error',
    ...
    evaluators: [
      {
        ...
        evaluate: ({ output, expected: required }) => {
          ...
          return {
            score: passed ? 1 : 0,
            passed,
            code: passed ? 'required_events_present' : 'required_events_missing',
          };
        },
      },
    ],
...
function referenceWorkflowSpecSource(packageScope: string): string {
...
import { createStructuredLogger, type LogRecord } from '${packageScope}/observability';
...
async function collect(config: ReferenceAiWorkflowConfig, request: ReferenceAiWorkflowRequest) {
  const events = [];
  for await (const event of runReferenceAiWorkflow(config, request)) events.push(event);
  return events;
}
...
describe('generated AI reference workflow', () => {
  it('streams a governed typed tool workflow with durable state and evaluation evidence', async () => {
    ...
    expect(await setup.persistence.get('run-1')).toMatchObject({
      ...
      checkpoint: {
        stepId: 'typed_tool',
        state: { stage: 'tool_completed', toolId: REFERENCE_LOOKUP_TOOL_ID },
      },
    });
    expect(setup.auditEvents.map((event) => event.type)).toContain('model_route');
    expect(setup.auditEvents.map((event) => event.type)).toContain('content_policy');
    expect(setup.auditEvents.map((event) => event.type)).toContain('tool_policy');
  ...
  it('requires trusted approval before the side-effect boundary can proceed', async () => {
    ...
    expect(await setup.persistence.get('run-1')).toMatchObject({ status: 'completed' });
